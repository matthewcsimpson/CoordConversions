...
describe('Conversion Functions', () => {
  ...
  test('dmToDD - basic conversion', () => {
    const dm = { kind: CoordinateType.LAT, degrees: 48, minutes: 51.264, hemi: Hemisphere.N };
    const dd = dmToDD(dm);
    expect(dd.kind).toBe(CoordinateType.LAT);
    expect(dd.degrees).toBeCloseTo(48.8544, 6);
  });
  ...
  test('dmsToDD - basic conversion', () => {
    const dms = { kind: CoordinateType.LAT, degrees: 48, minutes: 51, seconds: 15.84, hemi: Hemisphere.N };
...
describe('Formatting Functions', () => {
  ...
  const testDM = { kind: CoordinateType.LAT, degrees: 48, minutes: 51.264, hemi: Hemisphere.N };
  const testDMS = { kind: CoordinateType.LAT, degrees: 48, minutes: 51, seconds: 15.84, hemi: Hemisphere.N };
...
describe('Pair Functions', () => {
  ...
  test('dmPairToDD', () => {
    const latDM = { kind: CoordinateType.LAT, degrees: 48, minutes: 51.264, hemi: Hemisphere.N };
...
describe('Error Handling', () => {
  test('parseToDD - invalid format', () => {
    expect(() => parseToDD('invalid format', CoordinateType.LAT)).toThrow('Unrecognized coordinate format');
  });

  test('parseToDD - latitude out of range', () => {
    expect(() => parseToDD('91', CoordinateType.LAT)).toThrow('lat degrees out of range: 91');
    expect(() => parseToDD('-91', CoordinateType.LAT)).toThrow('lat degrees out of range: -91');
  });

  test('parseToDD - longitude out of range', () => {
    expect(() => parseToDD('181', CoordinateType.LON)).toThrow('lon degrees out of range: 181');
    expect(() => parseToDD('-181', CoordinateType.LON)).toThrow('lon degrees out of range: -181');
  });

  test('parseToDD - minutes out of range', () => {
    expect(() => parseToDD('45° 65\' N', CoordinateType.LAT)).toThrow('Minutes must be < 60');
  });

  test('parseToDD - seconds out of range', () => {
    expect(() => parseToDD('45° 7\' 65" N', CoordinateType.LAT)).toThrow('Seconds must be < 60');
  });

  test('dmToDD - minutes out of range', () => {
    const dm = { kind: CoordinateType.LAT, degrees: 45, minutes: 65, hemi: Hemisphere.N };
    expect(() => dmToDD(dm)).toThrow('Minutes must be in [0, 60)');
  });

  test('dmsToDD - minutes out of range', () => {
    const dms = { kind: CoordinateType.LAT, degrees: 45, minutes: 65, seconds: 30, hemi: Hemisphere.N };
    expect(() => dmsToDD(dms)).toThrow('Minutes must be in [0, 60)');
  });

  test('dmsToDD - seconds out of range', () => {
    const dms = { kind: CoordinateType.LAT, degrees: 45, minutes: 30, seconds: 65, hemi: Hemisphere.N };
